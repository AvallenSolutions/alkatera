...
/**
 * GET /api/orchards/[id]/spray-chemicals?growing_profile_id=XXX
 *
...
export async function GET(
  ...
  try {
    ...
    if (error) {
      console.error('[OrchardSprayChemicals GET] Query error:', error);
      return NextResponse.json({ error: 'Internal server error' }, { status: 500 });
    }

    return NextResponse.json({ data: data || [] });
  } catch (err: any) {
    console.error('[OrchardSprayChemicals GET] Error:', err);
    return NextResponse.json({ error: 'Internal server error' }, { status: 500 });
  }
}

/**
 * POST /api/orchards/[id]/spray-chemicals
 *
 ...
 *   chemicals: OrchardSprayChemicalDraft[]
 */
export async function POST(
  ...
  try {
    ...
    if (deleteError) {
      console.error('[OrchardSprayChemicals POST] Delete error:', deleteError);
      return NextResponse.json({ error: 'Internal server error' }, { status: 500 });
    }
    ...
    if (error) {
      console.error('[OrchardSprayChemicals POST] Insert error:', error);
      return NextResponse.json({ error: 'Internal server error' }, { status: 500 });
    }

    return NextResponse.json({ success: true, data }, { status: 201 });
  } catch (err: any) {
    console.error('[OrchardSprayChemicals POST] Error:', err);
    return NextResponse.json({ error: 'Internal server error' }, { status: 500 });
  }
}

/**
 * DELETE /api/orchards/[id]/spray-chemicals?growing_profile_id=XXX
 *
...
export async function DELETE(
  ...
  try {
    ...
    if (error) {
      console.error('[OrchardSprayChemicals DELETE] Error:', error);
      return NextResponse.json({ error: 'Internal server error' }, { status: 500 });
    }

    return NextResponse.json({ success: true });
  } catch (err: any) {
    console.error('[OrchardSprayChemicals DELETE] Error:', err);
    return NextResponse.json({ error: 'Internal server error' }, { status: 500 });
  }
}
