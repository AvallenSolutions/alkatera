...
/**
 * GET /api/procurement/[slug]/sku-lists
 * Lists every procurement SKU list upload for the caller's procurement org.
 */
...
/**
 * POST /api/procurement/[slug]/sku-lists
 * Multipart form with one "file" field. Uploads to the procurement-sku-lists
 * Storage bucket and inserts a procurement_sku_lists row with status='pending'.
 * Returns the new row's id so the client can call /parse next.
 *
 * Only procurement owners can upload.
 */
export async function POST(request: Request, { params }: { params: { slug: string } }) {
  ...
  const { data: row, error: insertError } = await auth.supabase
    .from('procurement_sku_lists')
    .insert({
      procurement_org_id: auth.organization.id,
  ...
  if (insertError || !row) {
    await auth.supabase.storage.from('procurement-sku-lists').remove([storagePath]);
    return NextResponse.json(
