...
/**
 * GET /api/arable-fields/[id]/growing-profile/evidence?profile_id=xxx
...
export async function GET(
  ...
  try {
    ...
    if (error) {
      console.error('[ArableSoilEvidence GET] Query error:', error);
    ...
    const withUrls = await Promise.all(
      (data || []).map(async (doc) => {
        const { data: signedUrlData } = await supabase.storage
          .from('arable-soil-carbon-evidence')
    ...
    console.error('[ArableSoilEvidence GET] Error:', err);
    return NextResponse.json({ error: 'Internal server error' }, { status: 500 });
  }
}

/**
 * POST /api/arable-fields/[id]/growing-profile/evidence
...
export async function POST(
  ...
  try {
    ...
    if (uploadError) {
      console.error('[ArableSoilEvidence POST] Upload error:', uploadError);
    ...
    if (error) {
      console.error('[ArableSoilEvidence POST] Insert error:', error);
    ...
    console.error('[ArableSoilEvidence POST] Error:', err);
    return NextResponse.json({ error: 'Internal server error' }, { status: 500 });
  }
}

/**
 * DELETE /api/arable-fields/[id]/growing-profile/evidence?evidence_id=xxx
...
export async function DELETE(
  ...
  try {
    ...
    console.error('[ArableSoilEvidence DELETE] Error:', err);
    return NextResponse.json({ error: 'Internal server error' }, { status: 500 });
  }
}
