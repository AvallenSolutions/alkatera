...
import { buildRosaChatPersona, buildRosaPageContextBlock } from '@/lib/rosa/persona';
...
// The persona itself lives in lib/rosa/persona.ts, composed from named blocks
// shared with every other Rosa surface. This route supplies only what is
// runtime-specific: today's date, the memory block and the page context.
...
export async function POST(request: NextRequest) {
  ...
  // rather than straight into the prompt.
  const systemPrompt = buildRosaChatPersona({
    today: new Date().toISOString().slice(0, 10),
    memoryBlock,
    pageContextBlock: buildRosaPageContextBlock(pageContext),
  });
