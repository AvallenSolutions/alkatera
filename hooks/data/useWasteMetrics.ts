...
export function useWasteMetrics(year?: number) {
  ...
  const fetchWasteData = useCallback(async () => {
    ...
    try {
      ...
      const totalWasteKg = wasteEntries.reduce((sum, w) => {
        const qty = Number(w.quantity || 0);
        return sum + (isNaN(qty) ? 0 : qty);
      }, 0);
      const totalEmissions = wasteEntries.reduce((sum, w) => {
        const emissions = Number(w.calculated_emissions_kg_co2e || 0);
      ...
      const wasteStreams: WasteStreamItem[] = wasteEntries.map((entry) => ({
        ...
        total_quantity_kg: Number(entry.quantity || 0) || 0,
        avg_recovery_percentage: Number(entry.waste_recovery_percentage || 0) || 0,
        total_emissions_kg_co2e: Number(entry.calculated_emissions_kg_co2e || 0) || 0,
        entry_count: 1,
        data_provenance: entry.data_provenance || 'unknown',
        confidence_score: Number(entry.confidence_score || 0) || 0,
      }));
      ...
      const endOfLifeScenarios: EndOfLifeScenario[] = (eolData || []).map((scenario: any) => ({
        ...
        recycling_percentage: Number(scenario.recycling_percentage || 0) || 0,
        landfill_percentage: Number(scenario.landfill_percentage || 0) || 0,
        incineration_percentage: Number(scenario.incineration_percentage || 0) || 0,
        composting_percentage: Number(scenario.composting_percentage || 0) || 0,
        reuse_percentage: Number(scenario.reuse_percentage || 0) || 0,
        total_emissions_kg_co2e: Number(scenario.total_emissions_kg_co2e || 0) || 0,
        avoided_emissions_kg_co2e: Number(scenario.avoided_emissions_kg_co2e || 0) || 0,
      }));

      const circularityMetrics: CircularityMetrics | null = circularityData ? {
        total_products: Number(circularityData.total_products || 0) || 0,
        avg_packaging_circularity: Number(circularityData.avg_packaging_circularity || 0) || 0,
        avg_mci: Number(circularityData.avg_mci || 0) || 0,
        avg_recycled_content: Number(circularityData.avg_recycled_content || 0) || 0,
        avg_recyclability: Number(circularityData.avg_recyclability || 0) || 0,
        reusable_materials_percentage: Number(circularityData.reusable_materials_percentage || 0) || 0,
        compostable_materials_percentage: Number(circularityData.compostable_materials_percentage || 0) || 0,
        total_materials_assessed: Number(circularityData.total_materials_assessed || 0) || 0,
      } : null;

      const targets: CircularityTarget | null = targetsData ? {
        id: targetsData.id,
        target_year: Number(targetsData.target_year) || new Date().getFullYear(),
        waste_diversion_target: Number(targetsData.waste_diversion_target || 0) || 0,
        recycled_content_target: Number(targetsData.recycled_content_target || 0) || 0,
        circularity_score_target: Number(targetsData.circularity_score_target || 0) || 0,
        packaging_recyclability_target: Number(targetsData.packaging_recyclability_target || 0) || 0,
