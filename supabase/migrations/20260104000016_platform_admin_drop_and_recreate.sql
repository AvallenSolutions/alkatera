/*
  # Platform Admin System - Drop and Recreate Functions

  First drops existing functions, then recreates with fixed column names
*/
...
DROP FUNCTION IF EXISTS add_platform_admin(text);
DROP FUNCTION IF EXISTS remove_platform_admin(text);
...
CREATE FUNCTION add_platform_admin(user_email text)
RETURNS TABLE (
  success boolean,
  message text,
  result_user_id uuid,
...
CREATE FUNCTION remove_platform_admin(user_email text)
RETURNS TABLE (
  success boolean,
  message text
)
...
CREATE FUNCTION list_platform_admins()
RETURNS TABLE (
  result_user_id uuid,
  email text,
  full_name text,
