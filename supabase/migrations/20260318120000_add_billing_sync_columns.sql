-- Migration: Add billing sync columns and update subscription RPC
-- Adds current_period_end for accurate next billing date display
-- Adds billing address columns for Stripe-synced billing details
-- Updates update_subscription_from_stripe to accept current_period_end

-- Add current_period_end to track Stripe subscription billing cycle
ALTER TABLE public.organizations
  ADD COLUMN IF NOT EXISTS current_period_end TIMESTAMPTZ;

-- Add billing address columns (separate from org location/address fields)
ALTER TABLE public.organizations
...
-- Ensure tax_id column exists (referenced by OrganisationSettings but may be missing)
ALTER TABLE public.organizations
  ADD COLUMN IF NOT EXISTS tax_id TEXT;

-- Recreate update_subscription_from_stripe with current_period_end support
