...
VALUES
  ('canopy', 3, 'Canopy', 'Comprehensive sustainability management for established organisations',
   ...
   599, 5990, '["advanced_data_quality","api_access","audit_packages","automated_verification","b_corp_assessment","bcorp_tracking","biodiversity_tracking","carbon_footprint_ghg","cdp_tracking","community_charitable_giving","community_impact_stories","community_local_impact","community_volunteering","company_emissions_current","csrd_compliance","custom_weighting","dashboard_vitality","ef_31","ef_31_single_score","email_support","facilities_management","fleet_overview","fleet_reporting","full_scope_3","gap_analysis","ghg_emissions","governance_ethics","greenwash_documents","greenwash_unlimited","greenwash_website","gri_standards","iso_14001","iso_50001","knowledge_bank_manage","knowledge_bank_read","land_use_impact","lca_distribution","lca_end_of_life","lca_use_phase","live_passport","monthly_analytics","pdf_report_export","pef_reports","people_diversity_inclusion","people_fair_work","people_training","people_wellbeing","priority_chat","product_comparison","product_management","product_passport","recipe_2016","resource_use_tracking","rosa_ai_unlimited","sandbox_analytics","sbti_targets","supplier_directory","supply_chain_mapping","third_party_verification","vehicle_registry","verified_data","waste_circularity","water_footprint","white_label","year_over_year"]'::jsonb, true, now())
ON CONFLICT (tier_name) DO UPDATE SET
  tier_level = EXCLUDED.tier_level,
  display_name = EXCLUDED.display_name,
  description = EXCLUDED.description,
