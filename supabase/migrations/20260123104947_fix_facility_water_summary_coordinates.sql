-- Fix facility_water_summary view to use correct coordinate column names
-- The view was referencing f.latitude/f.longitude but the facilities table
-- uses address_lat/address_lng columns. This caused coordinates to be NULL
-- and prevented facilities from appearing on water risk maps.
...
COMMENT ON VIEW public.facility_water_summary IS 'Aggregated water metrics per facility with AWARE risk assessment. Uses address_lat/address_lng for coordinates.';
