...
-- ============================================================================
-- STEP 3: Create LCA Calculation Logs Table (separate from GHG calculation_logs)
-- ============================================================================
-- NOTE: A separate calculation_logs table already exists for GHG calculations
-- (created by 20251109112847_create_calculation_logs_table.sql).
-- This creates a separate table specifically for LCA calculations.

CREATE TABLE IF NOT EXISTS public.lca_calculation_logs (
...
ALTER TABLE public.lca_calculation_logs ENABLE ROW LEVEL SECURITY;

COMMENT ON TABLE public.lca_calculation_logs IS 'Immutable audit log of all LCA calculations, capturing inputs, outputs, governance metadata, and engine version for complete reproducibility.';
COMMENT ON COLUMN public.lca_calculation_logs.result_payload IS 'Final calculation results including aggregated impacts, emissions factors, and summary statistics.';
COMMENT ON COLUMN public.lca_calculation_logs.inputs_payload IS 'Complete set of flattened data points used as inputs, including all nested LCA dependencies.';
COMMENT ON COLUMN public.lca_calculation_logs.governance_metadata IS 'Metadata about governance checks performed: DQI profile, boundary compatibility, validation results.';
COMMENT ON COLUMN public.lca_calculation_logs.calculation_engine_version IS 'Version identifier of the calculation engine, enabling reproducibility and change tracking.';
COMMENT ON COLUMN public.lca_calculation_logs.error_details IS 'Detailed error information if calculation failed, including error type and message for debugging.';

-- Create indexes for efficient log queries
CREATE INDEX IF NOT EXISTS idx_lca_calculation_logs_lca_id
  ON public.lca_calculation_logs(lca_id);

CREATE INDEX IF NOT EXISTS idx_lca_calculation_logs_organization_id
  ON public.lca_calculation_logs(organization_id);

CREATE INDEX IF NOT EXISTS idx_lca_calculation_logs_created_at
  ON public.lca_calculation_logs(created_at DESC);

CREATE INDEX IF NOT EXISTS idx_lca_calculation_logs_status
  ON public.lca_calculation_logs(status);

-- RLS policies for LCA calculation logs
CREATE POLICY "Organization members can view LCA calculation logs"
  ON public.lca_calculation_logs
  FOR SELECT
  TO authenticated
  USING (organization_id = get_current_organization_id());

CREATE POLICY "Organization members can insert LCA calculation logs"
  ON public.lca_calculation_logs
  FOR INSERT
  TO authenticated
