...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const { data: emissionsFactor, error: factorError } = await supabaseClient
      ...
      .eq("category", "Scope 3")
      .eq("type", "Category 5 - Incineration")
    ...
    if (!emissionsFactor) {
      return new Response(
        JSON.stringify({ 
          error: `No Scope 3 Category 5 incineration emissions factor found for waste type: ${activity_data.waste_type}`,
    ...
    return new Response(
      JSON.stringify({
        ...
        metadata: {
          ...
          calculation_type: "Scope 3: Category 5 - Waste Generated in Operations - Incineration",
        },
      }),
      {
        status: 200,
