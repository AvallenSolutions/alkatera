...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const { data: emissionsFactor, error: factorError } = await supabaseClient
      ...
      .eq("category", "Scope 2")
      .eq("type", "Location-Based - Electricity")
    ...
    if (!emissionsFactor) {
      return new Response(
        JSON.stringify({ 
          error: `No Scope 2 Location-Based Electricity emissions factor found for grid region: ${activity_data.grid_region}`,
    ...
    return new Response(
      JSON.stringify({
        ...
        metadata: {
          ...
          calculation_type: "Scope 2: Location-Based",
        },
      }),
      {
        status: 200,
