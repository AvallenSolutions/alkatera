...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (typeof activity_data.kwh !== "number" || activity_data.kwh <= 0) {
      return new Response(
        JSON.stringify({
          error: "Invalid input: activity_data.kwh is required and must be a positive number"
        }),
        {
          status: 400,
          headers: { ...corsHeaders, "Content-Type": "application/json" },
    ...
    if (!activity_data.region || typeof activity_data.region !== "string") {
      return new Response(
        JSON.stringify({
          error: "Invalid input: activity_data.region is required and must be a string"
        }),
        {
          status: 400,
          headers: { ...corsHeaders, "Content-Type": "application/json" },
    ...
    if (!emissionsFactor) {
      return new Response(
        JSON.stringify({
          error: `No Scope 2 Market-Based emissions factor found for region: ${activity_data.region}`,
    ...
    const emissions_tco2e = Number((
      activity_data.kwh * Number(emissionsFactor.value)
    ).toFixed(6));

    const inputPayload = {
      provenance_id: provenance_id,
      kwh: activity_data.kwh,
