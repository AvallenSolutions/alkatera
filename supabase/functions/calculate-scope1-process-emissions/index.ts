...
interface ActivityData {
  process_input_material: string;
  mass_input_tonnes: number;
}
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (!activity_data.process_input_material || typeof activity_data.process_input_material !== "string") {
      return new Response(
        JSON.stringify({ 
          error: "Invalid input: activity_data.process_input_material is required and must be a string" 
        }),
        {
          status: 400,
          headers: { ...corsHeaders, "Content-Type": "application/json" },
    ...
    if (typeof activity_data.mass_input_tonnes !== "number" || activity_data.mass_input_tonnes <= 0) {
      return new Response(
        JSON.stringify({ 
          error: "Invalid input: activity_data.mass_input_tonnes is required and must be a positive number" 
        }),
        {
          status: 400,
          headers: { ...corsHeaders, "Content-Type": "application/json" },
    ...
    const { data: emissionsFactor, error: factorError } = await supabaseClient
      ...
      .eq("category", "Scope 1")
      .eq("type", "Process Emissions")
      .eq("name", activity_data.process_input_material)
    ...
    if (!emissionsFactor) {
      return new Response(
        JSON.stringify({ 
          error: `No Scope 1 Process Emissions factor found for material: ${activity_data.process_input_material}`,
          message: "Please ensure emissions factors are loaded for this process material or contact support" 
        }),
        {
          status: 404,
          headers: { ...corsHeaders, "Content-Type": "application/json" },
    ...
    const emissions_tco2e = Number((
      activity_data.mass_input_tonnes * Number(emissionsFactor.value)
    ).toFixed(6));

    const inputPayload = {
      provenance_id: provenance_id,
      process_input_material: activity_data.process_input_material,
      mass_input_tonnes: activity_data.mass_input_tonnes,
