...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const { data: emissionsFactor, error: factorError } = await supabaseClient
      ...
      .eq("category", "Scope 2")
      .eq("type", "Location-Based - Cooling")
    ...
    if (!emissionsFactor) {
      return new Response(
        JSON.stringify({ 
          error: `No Scope 2 Location-Based Cooling emissions factor found for region: ${activity_data.grid_region}`,
    ...
    return new Response(
      JSON.stringify({
        ...
        metadata: {
          ...
          calculation_type: "Scope 2: Location-Based - Cooling",
        },
      }),
      {
        status: 200,
