...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (!emissionsFactor) {
      return new Response(
        JSON.stringify({ 
          error: `No Scope 2 Market-Based Heat emissions factor found for supplier or instrument: ${activity_data.supplier_or_instrument_type}`,
          message: "Please ensure emissions factors are loaded for this heat supplier or contractual instrument or contact support" 
        }),
        {
          status: 404,
          headers: { ...corsHeaders, "Content-Type": "application/json" },
    ...
    return new Response(
      JSON.stringify({
        ...
        metadata: {
          ...
          calculation_type: "Scope 2: Market-Based - Heat",
        },
      }),
      {
        status: 200,
