...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (!activity_data.energy_type || typeof activity_data.energy_type !== "string") {
      return new Response(
        JSON.stringify({ 
          error: "Invalid input: activity_data.energy_type is required and must be a string" 
        }),
        {
          status: 400,
          headers: { ...corsHeaders, "Content-Type": "application/json" },
    ...
    const { data: emissionsFactor, error: factorError } = await supabaseClient
      ...
      .eq("category", "Scope 3")
      .eq("type", "Category 3 - WTT - Purchased Energy")
      .eq("name", activity_data.energy_type)
    ...
    if (!emissionsFactor) {
      return new Response(
        JSON.stringify({ 
          error: `No Scope 3 Category 3 WTT emissions factor found for energy type: ${activity_data.energy_type}`,
          message: "Please ensure emissions factors are loaded for this energy type or contact support" 
        }),
        {
          status: 404,
          headers: { ...corsHeaders, "Content-Type": "application/json" },
    ...
    const inputPayload = {
      ...
      energy_type: activity_data.energy_type,
    ...
    return new Response(
      JSON.stringify({
        ...
        metadata: {
          energy_type: emissionsFactor.name,
          ...
          calculation_type: "Scope 3: Category 3 - Fuel & Energy Related - WTT Purchased Energy",
        },
      }),
      {
        status: 200,
