...
function buildInviteEmail(inviteLink: string, organizationName: string, roleName: string, siteUrl: string): string {
  ...
  const content = `
    ${studioParagraph(`You've been invited to join <strong>${escapeHtml(organizationName)}</strong> on alka<strong>tera</strong> as a <strong>${roleDisplay}</strong>.`)}
    ${studioParagraph(`alka<strong>tera</strong> is a sustainability platform that helps organisations measure, manage and report on their environmental impact.`)}
    ${studioButton(inviteLink, "Accept Invitation")}
    ${studioParagraph(`This invitation link will expire in 7 days.`)}
    ${studioCallout(
