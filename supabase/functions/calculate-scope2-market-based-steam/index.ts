...
interface ActivityData {
  supplier_or_instrument_type: string;
  energy_kwh: number;
}
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (!activity_data.supplier_or_instrument_type || typeof activity_data.supplier_or_instrument_type !== "string") {
      return new Response(
        JSON.stringify({ 
          error: "Invalid input: activity_data.supplier_or_instrument_type is required and must be a string" 
        }),
        {
          status: 400,
          headers: { ...corsHeaders, "Content-Type": "application/json" },
    ...
    const { data: emissionsFactor, error: factorError } = await supabaseClient
      ...
      .eq("category", "Scope 2")
      .eq("type", "Market-Based - Steam")
      .eq("name", activity_data.supplier_or_instrument_type)
    ...
    if (!emissionsFactor) {
      return new Response(
        JSON.stringify({ 
          error: `No Scope 2 Market-Based Steam emissions factor found for supplier or instrument: ${activity_data.supplier_or_instrument_type}`,
          message: "Please ensure emissions factors are loaded for this steam supplier or contractual instrument or contact support" 
        }),
        {
          status: 404,
          headers: { ...corsHeaders, "Content-Type": "application/json" },
    ...
    const inputPayload = {
      ...
      supplier_or_instrument_type: activity_data.supplier_or_instrument_type,
    ...
    return new Response(
      JSON.stringify({
        ...
        metadata: {
          supplier_or_instrument: emissionsFactor.name,
          ...
          calculation_type: "Scope 2: Market-Based - Steam",
        },
      }),
      {
        status: 200,
