...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (!activity_data.treatment_method || typeof activity_data.treatment_method !== "string") {
      return new Response(
        JSON.stringify({ 
          error: "Invalid input: activity_data.treatment_method is required and must be a string" 
        }),
        {
          status: 400,
          headers: { ...corsHeaders, "Content-Type": "application/json" },
    ...
    if (typeof activity_data.volume_m3 !== "number" || activity_data.volume_m3 <= 0) {
      return new Response(
        JSON.stringify({ 
          error: "Invalid input: activity_data.volume_m3 is required and must be a positive number" 
        }),
        {
          status: 400,
          headers: { ...corsHeaders, "Content-Type": "application/json" },
    ...
    const { data: emissionsFactor, error: factorError } = await supabaseClient
      ...
      .eq("category", "Scope 3")
      .eq("type", "Category 5 - Wastewater")
      .eq("name", activity_data.treatment_method)
    ...
    if (!emissionsFactor) {
      return new Response(
        JSON.stringify({ 
          error: `No Scope 3 Category 5 wastewater emissions factor found for treatment method: ${activity_data.treatment_method}`,
          message: "Please ensure emissions factors are loaded for this treatment method or contact support" 
        }),
        {
          status: 404,
          headers: { ...corsHeaders, "Content-Type": "application/json" },
    ...
    const emissions_tco2e = Number((
      (activity_data.volume_m3 * Number(emissionsFactor.value)) / 1000
    ).toFixed(6));

    const inputPayload = {
      provenance_id: provenance_id,
      treatment_method: activity_data.treatment_method,
      volume_m3: activity_data.volume_m3,
    ...
    return new Response(
      JSON.stringify({
        ...
        metadata: {
          treatment_method: emissionsFactor.name,
          ...
          calculation_type: "Scope 3: Category 5 - Waste Generated in Operations - Wastewater Treatment",
        },
      }),
      {
        status: 200,
