...
// Currency conversion rates to GBP (base currency)
...
function normaliseSpendToGBP(spend: number, currency: string): number {
  const upperCurrency = currency.toUpperCase();
  const rate = CURRENCY_RATES[upperCurrency];
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (!activity_data.item_category || typeof activity_data.item_category !== "string") {
      return new Response(
        JSON.stringify({ 
          error: "Invalid input: activity_data.item_category is required and must be a string" 
        }),
        {
          status: 400,
          headers: { ...corsHeaders, "Content-Type": "application/json" },
    ...
    if (!activity_data.currency || typeof activity_data.currency !== "string") {
      return new Response(
        JSON.stringify({ 
          error: "Invalid input: activity_data.currency is required and must be a string" 
        }),
        {
          status: 400,
          headers: { ...corsHeaders, "Content-Type": "application/json" },
    ...
    // Normalise spend to GBP
    let normalised_spend_gbp: number;
    try {
      normalised_spend_gbp = normaliseSpendToGBP(activity_data.spend, activity_data.currency);
    } catch (error) {
      return new Response(
        JSON.stringify({ 
          error: error instanceof Error ? error.message : "Failed to normalise currency",
    ...
    const { data: emissionsFactor, error: factorError } = await supabaseClient
      ...
      .eq("category", "Scope 3")
      .eq("type", "Category 2 - Capital Goods - Spend")
      .eq("name", activity_data.item_category)
    ...
    if (!emissionsFactor) {
      return new Response(
        JSON.stringify({ 
          error: `No Scope 3 Category 2 Capital Goods emissions factor found for item category: ${activity_data.item_category}`,
          message: "Please ensure emissions factors are loaded for this capital goods category or contact support" 
        }),
        {
          status: 404,
          headers: { ...corsHeaders, "Content-Type": "application/json" },
    ...
    // Calculate emissions: (normalised_spend_gbp * emissions_factor_value) / 1000000
    // Factor is in kgCO2e per million GBP, so divide by 1,000,000 to get per GBP, then divide by 1000 to convert kg to tonnes
    const emissions_tco2e = Number((
      (normalised_spend_gbp * Number(emissionsFactor.value)) / 1000000
    ).toFixed(6));

    const inputPayload = {
      provenance_id: provenance_id,
      item_category: activity_data.item_category,
      spend: activity_data.spend,
      currency: activity_data.currency,
      normalised_spend_gbp: normalised_spend_gbp,
    ...
    return new Response(
      JSON.stringify({
        ...
        metadata: {
          item_category: emissionsFactor.name,
          spend_original: activity_data.spend,
          currency_original: activity_data.currency,
          spend_normalised_gbp: normalised_spend_gbp,
          ...
          calculation_type: "Scope 3: Category 2 - Capital Goods",
        },
      }),
      {
        status: 200,
