...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (!emissionsFactor) {
      return new Response(
        JSON.stringify({ 
          error: `No Scope 3 Category 7 employee commuting emissions factor found for vehicle type: ${activity_data.vehicle_type}`,
    ...
    return new Response(
      JSON.stringify({
        ...
        metadata: {
          ...
          calculation_type: "Scope 3: Category 7 - Employee Commuting - Distance",
        },
      }),
      {
        status: 200,
