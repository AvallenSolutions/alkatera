...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (!emissionsFactor) {
      return new Response(
        JSON.stringify({ 
          error: `No Scope 3 Category 3 WTT emissions factor found for fuel type: ${activity_data.fuel_type}`,
    ...
    return new Response(
      JSON.stringify({
        ...
        metadata: {
          fuel_type: emissionsFactor.name,
          ...
          calculation_type: "Scope 3: Category 3 - Fuel & Energy Related - WTT Energy",
        },
      }),
      {
        status: 200,
