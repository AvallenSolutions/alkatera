...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (!activity_data.waste_type || typeof activity_data.waste_type !== "string") {
      return new Response(
        JSON.stringify({ 
          error: "Invalid input: activity_data.waste_type is required and must be a string" 
        }),
        {
          status: 400,
          headers: { ...corsHeaders, "Content-Type": "application/json" },
    ...
    if (typeof activity_data.mass_tonnes !== "number" || activity_data.mass_tonnes <= 0) {
      return new Response(
        JSON.stringify({ 
          error: "Invalid input: activity_data.mass_tonnes is required and must be a positive number" 
        }),
        {
          status: 400,
          headers: { ...corsHeaders, "Content-Type": "application/json" },
    ...
    const { data: emissionsFactor, error: factorError } = await supabaseClient
      ...
      .eq("category", "Scope 3")
      .eq("type", "Category 5 - Anaerobic Digestion")
      .eq("name", activity_data.waste_type)
    ...
    if (!emissionsFactor) {
      return new Response(
        JSON.stringify({ 
          error: `No Scope 3 Category 5 anaerobic digestion emissions factor found for waste type: ${activity_data.waste_type}`,
          message: "Please ensure emissions factors are loaded for this waste type or contact support" 
        }),
        {
          status: 404,
          headers: { ...corsHeaders, "Content-Type": "application/json" },
    ...
    const emissions_tco2e = Number((
      activity_data.mass_tonnes * Number(emissionsFactor.value)
    ).toFixed(6));

    const inputPayload = {
      provenance_id: provenance_id,
      waste_type: activity_data.waste_type,
      mass_tonnes: activity_data.mass_tonnes,
    ...
    return new Response(
      JSON.stringify({
        ...
        metadata: {
          waste_type: emissionsFactor.name,
          ...
          calculation_type: "Scope 3: Category 5 - Waste Generated in Operations - Anaerobic Digestion",
        },
      }),
      {
        status: 200,
