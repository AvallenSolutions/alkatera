...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (typeof activity_data.fuel_mass_tonnes !== "number" || activity_data.fuel_mass_tonnes <= 0) {
      return new Response(
        JSON.stringify({ 
          error: "Invalid input: activity_data.fuel_mass_tonnes is required and must be a positive number" 
        }),
        {
          status: 400,
          headers: { ...corsHeaders, "Content-Type": "application/json" },
    ...
    const { data: emissionsFactor, error: factorError } = await supabaseClient
      ...
      .eq("category", "Scope 1")
      .eq("type", "Stationary Combustion - Mass")
    ...
    const emissions_tco2e = Number((
      activity_data.fuel_mass_tonnes * Number(emissionsFactor.value)
    ).toFixed(6));

    const inputPayload = {
      provenance_id: provenance_id,
      fuel_type: activity_data.fuel_type,
      fuel_mass_tonnes: activity_data.fuel_mass_tonnes,
