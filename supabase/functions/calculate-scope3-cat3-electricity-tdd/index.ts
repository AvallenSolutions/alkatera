...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (!emissionsFactor) {
      return new Response(
        JSON.stringify({ 
          error: `No Scope 3 Category 3 T&D Losses emissions factor found for grid region: ${activity_data.grid_region}`,
          message: "Please ensure emissions factors are loaded for this grid region or contact support" 
        }),
        {
          status: 404,
          headers: { ...corsHeaders, "Content-Type": "application/json" },
    ...
    return new Response(
      JSON.stringify({
        ...
        metadata: {
          ...
          calculation_type: "Scope 3: Category 3 - Fuel & Energy Related - T&D Losses",
        },
      }),
      {
        status: 200,
