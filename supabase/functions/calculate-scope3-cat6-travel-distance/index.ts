...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const { data: emissionsFactor, error: factorError } = await supabaseClient
      ...
      .eq("category", "Scope 3")
      .eq("type", "Category 6 - Business Travel - Distance")
    ...
    if (!emissionsFactor) {
      return new Response(
        JSON.stringify({ 
          error: `No Scope 3 Category 6 business travel emissions factor found for vehicle type: ${activity_data.vehicle_type}`,
    ...
    const emissions_tco2e = Number((
      (activity_data.distance_km * Number(emissionsFactor.value)) / 1000
    ).toFixed(6));
    ...
    return new Response(
      JSON.stringify({
        ...
        metadata: {
          vehicle_type: emissionsFactor.name,
          ...
          calculation_type: "Scope 3: Category 6 - Business Travel - Distance",
        },
      }),
      {
        status: 200,
