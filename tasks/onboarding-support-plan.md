...
### Sync log (main → redesign)

- **2026-07-16** (merge 665da41e): main b53039eb..0132d992, 45 commits. Material-input fixes (openlca_no_match, recycled-content declared zeros), hospitality July programme, smart-upload learning, archiving, security fixes. Rule applied: main wins logic, redesign wins presentation. `components/hospitality/RecipeEditor.tsx` taken main-verbatim, NEEDS STUDIO RESTYLE. 19 migrations applied to local DB. Two files main's routes import were untracked in the main repo; committed to main as ab43aa3f.

**The routine:** work only in this worktree on branch `redesign`; never merge redesign → main before the go-live pass; after notable fixes land on main (or weekly), run `git merge main` here, resolve with the logic-vs-presentation rule, apply new migrations to the local DB additively (no db reset), typecheck + scoped tests + browser smoke, commit the merge, and add a line to this log.

### Future work (parked, out of scope for this pass)

- ~~Server-side scheduled nudges/drip~~ **DONE 2026-07-12** (commit 3922c885): `growthStallSweep` Inngest function, weekly Netlify heartbeat (Mondays 08:00 UTC), `growth_score_snapshots` table (migration 20260712170000, PENDING PROD), nudges all org members via `user_notifications` with 14-day dedupe.
- ~~Converging legacy guides onto Coachmark~~ **DONE 2026-07-12** (commit 9b117e90): FactorInfoHint renders through the new `CoachmarkBody` shell; RecipeSidebarTour, DashboardGuide, ProductGuide chrome restyled to studio; SearchGuidePanel was already studio; shared `EmptyState` now uses the studio Panel. All state flags and trigger logic unchanged.
- ~~Member/advisor flow restyle~~ **DONE 2026-07-12** (commit 1a54a862): both flows now use the arrival ritual's quiet shell (`isQuietFlow` in OnboardingWizard) and studio components throughout; logic untouched. Not yet walked in the browser with a real member/advisor login.
- ~~Arrival flow live click-through~~ **DONE 2026-07-12**: walked end to end in the browser against a fresh org (Fresh Start Drinks, seeded on local Supabase). All five screens render in studio language with the mono step counter; the persona choice persists to `rosa_memory` (`persona=sustainability`) and re-orders the desk (cellar promoted); the website reveal scraped avallenspirits.com and created 3 draft products; the estimate (~15 t CO2e) closed on "Your forest has started."; the desk landed never-empty with "Get your desk started" actions and the forest pill at score 4. Not yet exercised: Rosa live conversation (no GEMINI_API_KEY locally), the brand-colour "paint my house" path (kept alkatera's colours), and a true cold sign-up (tested via org switch, which also exercised the switcher).
