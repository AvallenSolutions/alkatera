...
- **Job 1 — scrape background fix SHIPPED** (74640aea). The prior session's uncommitted draft was complete and sound; kept it. The no-Inngest branch now returns the jobId immediately (202) and runs `runImportFromUrl` in the BACKGROUND via `@vercel/functions` `waitUntil` (NOT `unstable_after` — Next 14.2.5 predates `after`; confirmed absent from the package), using a service-role client (`service()` now exported from lib/inngest/functions/product-import.ts). `maxDuration=300`, Inngest branch untouched. `@vercel/functions` added to package.json + lockfile. Typecheck clean.
  - **Live re-test still OWED, BLOCKED.** Staging is now behind Vercel Deployment Protection (the branch alias serves Vercel's own SSO login), so the in-app browser can't reach the app to trigger a fresh import — it passes transparently only in Tim's own Vercel-authed browser. The one row in staging `product_import_jobs` is the PRE-FIX stranded job (avallenspirits.com, 16:36Z, status=failed, "took too long", updated_at==created_at → janitor-marked, worker never ran) — i.e. exactly the bug. To confirm the fix: Tim submits avallenspirits.com on staging from his own browser; watch `product_import_jobs` go pending→completed (Supabase MCP project vwhdyqvlgjqmlzmsvaes).
...
1. **`components/pulse/PulseCard.tsx`** (~210 LOC, ONE file) — shared chrome for all 21 Pulse grid cards across `/pulse` AND `/pulse/financial`, the DAILY Today surface. Headline `text-2xl font-semibold` → BigNumber/font-display (`:183`), label → Eyebrow, `border-border bg-card` → `border-studio-hairline bg-studio-paper` (`:102-106`), drop the decorative lucide header glyph (`:131`). Converts the whole glance layer in one edit. **Highest value/effort ratio in the app.**
