...
- **Access model reversed (same day)**: Tim decided the wiki is a subscriber tool, not a marketing asset. Moved to `app/(authenticated)/wiki` (same `/wiki` URL), server-side session gate, AppLayout tier/lifecycle handling, all subscriber tiers incl. trial. Marketing banner, sitemap entries and crawler access removed; sidebar + command palette entries added. Front end is a connected map (typed pill bands, connection curves, detail drawer, tours) with an in-place reader popout so users never leave `/wiki`.
- **Not done yet**: quarterly legislation review (due early October 2026); embeddings-based knowledge search (already flagged in Rosa's code).
