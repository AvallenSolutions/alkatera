...
export function CountrySelect({
  ...
  const triggerClasses = dark
    ? 'flex w-full items-center justify-between rounded-md border border-white/10 bg-white/5 px-3 py-2 text-sm text-white placeholder:text-white/30 hover:bg-white/10 focus:outline-none focus:ring-2 focus:ring-[#ccff00]/50 disabled:opacity-50 disabled:cursor-not-allowed'
        ...
        <div className="max-h-64 overflow-y-auto py-1">
          {filtered.length === 0 ? (
            ...
            filtered.map(c => (
              ...
              >
                <span>{c.label}</span>
                {c.value === value && <Check className="h-4 w-4 text-[#ccff00]" />}
              </button>
            ))
          )}
        </div>
