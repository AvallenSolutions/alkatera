...
/**
 * The vintage history, re-cut for the studio: a quiet mono count, a
 * room pill for the add action, the table on a cream hairline panel
 * with mono heads, typographic deltas and expand marks instead of
 * icons. Sorting, deltas and the expanded detail are unchanged.
 */
...
interface VintageHistoryTableProps {
  vintageImpacts: VintageImpactSummary[];
  vineyardId: string;
  onEditVintage: (year: number) => void;
  onAddVintage: () => void;
}
...
export function VintageHistoryTable({
  vintageImpacts,
  vineyardId,
  onEditVintage,
  onAddVintage,
}: VintageHistoryTableProps) {
  const [expandedYear, setExpandedYear] = useState<number | null>(null);

  // Sort descending for display (most recent first)
  const sorted = [...vintageImpacts].sort((a, b) => b.vintage_year - a.vintage_year);
  // For delta calculation, keep ascending order lookup
  const ascending = [...vintageImpacts].sort((a, b) => a.vintage_year - b.vintage_year);

  function getPrevious(year: number): VintageImpactSummary | undefined {
    const idx = ascending.findIndex((v) => v.vintage_year === year);
    return idx > 0 ? ascending[idx - 1] : undefined;
  }

  // Suggest next year to add
  const currentYear = new Date().getFullYear();
  const mostRecentYear = sorted.length > 0 ? sorted[0].vintage_year : currentYear - 1;
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between gap-4">
        <p className="font-mono text-[10px] font-bold uppercase tracking-[0.18em] text-muted-foreground">
          {sorted.length} vintage{sorted.length !== 1 ? 's' : ''} recorded
        </p>
        <PillButton variant="room" size="sm" onClick={onAddVintage}>
          Add vintage
          {suggestedYear && (
            ...
            {sorted.length === 0 ? (
              <TableRow>
                <TableCell colSpan={9} className="text-center text-muted-foreground py-8">
                  No vintages recorded yet. Add your first vintage to get started.
                </TableCell>
              </TableRow>
            ) : (
              sorted.map((v) => {
                const prev = getPrevious(v.vintage_year);
                const imp = v.impacts;
                const area = imp.flag_emissions.land_use_m2 / 10000 || 1;
                const ecotoxPerHa = imp.freshwater_ecotoxicity / area;
                const isExpanded = expandedYear === v.vintage_year;
                ...
                return (
                  <>
                    <TableRow
                      ...
                      onClick={() => setExpandedYear(isExpanded ? null : v.vintage_year)}
                    >
                      ...
                      <TableRow key={`${v.profile_id}-detail`}>
                        <TableCell colSpan={9} className="bg-muted/30 p-0">
                          <div className="px-6 py-4 grid grid-cols-2 md:grid-cols-4 gap-x-8 gap-y-3 text-xs">
                            ...
                            {/* Column 4: Normalised */}
                            <div className="space-y-1.5">
                              <p className="font-mono font-bold text-muted-foreground uppercase tracking-[0.18em] text-[10px]">Per kg Grapes</p>
