...
import { Info, AlertTriangle, Lock, Box, ArrowRight, Factory, Truck, Users, Trash2, Lightbulb, BookOpen, ChevronDown } from 'lucide-react';
import { useWizardContext } from '../WizardContext';
import { DEFAULT_PRODUCT_LOSS_CONFIG, type ProductLossConfig, getDefaultLossConfig, getDefaultConsumerWaste, getConsumerWasteEntry } from '@/lib/system-boundaries';
...
export function BoundaryStep() {
  ...
  // Category-aware default consumer waste
  const productCategory = preCalcState.product?.product_category || null;
  const productType = preCalcState.product?.product_type || null;
  const categoryDefault = getDefaultConsumerWaste(productCategory, productType);
  const wasteEntry = getConsumerWasteEntry(productCategory);

  // Expandable source methodology section
  ...
  // Auto-populate loss config with category-aware defaults when no config exists yet
  // and the boundary goes beyond cradle-to-gate.
  useEffect(() => {
    if (formData.systemBoundary === 'cradle-to-gate') return;
    if (formData.productLossConfig) return; // User already has config (loaded from DB or manually set)

    const defaultConfig = getDefaultLossConfig(productCategory, productType);
    updateField('productLossConfig', defaultConfig);
  }, [formData.systemBoundary, productCategory, productType]);
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Product Loss Rates — visible for boundaries beyond cradle-to-gate */}
      {formData.systemBoundary !== 'cradle-to-gate' && (
        <div className="space-y-3 rounded-lg border p-4">
          <div>
            <Label className="text-sm font-semibold">Product Loss Rates</Label>
            <p className="text-xs text-muted-foreground mt-1">
              Percentage of product lost at each downstream stage. Lost units still carry their
              full upstream carbon burden, which increases the per-unit footprint for units that
              reach the next stage.
              {productCategory
                ? ` Defaults are calibrated for ${productCategory}.`
                : ' Defaults are typical for the beverage industry.'}
            </p>
          </div>
          <div className="grid gap-4 sm:grid-cols-3">
            {boundaryNeedsDistribution(formData.systemBoundary) && (
              <div className="space-y-1.5">
                <Label htmlFor="distributionLoss" className="text-xs">
                  Distribution loss
                </Label>
                <div className="flex items-center gap-2">
                  <Input
                    ...
                    step={0.5}
                    className="w-20"
                    value={formData.productLossConfig?.distributionLossPercent ?? DEFAULT_PRODUCT_LOSS_CONFIG.distributionLossPercent}
                    onChange={(e) => {
                      const val = parseFloat(e.target.value) || 0;
                      updateField('productLossConfig', {
                        ...DEFAULT_PRODUCT_LOSS_CONFIG,
                        ...formData.productLossConfig,
                        distributionLossPercent: Math.min(50, Math.max(0, val)),
                      } as ProductLossConfig);
                    }}
                  />
                  <span className="text-xs text-muted-foreground">%</span>
                </div>
                <p className="text-[10px] text-muted-foreground">
                  Breakage, spoilage in transit
                  <span className="block mt-0.5 text-muted-foreground/70">
                    Source: Beverage industry average (WRAP, EU PEFCR)
                  </span>
                </p>
              </div>
            )}
            ...
            {boundaryNeedsUsePhase(formData.systemBoundary) && (
              <div className="space-y-1.5">
                <Label htmlFor="retailLoss" className="text-xs">
                  Retail loss
                </Label>
                <div className="flex items-center gap-2">
                  <Input
                    ...
                    step={0.5}
                    className="w-20"
                    value={formData.productLossConfig?.retailLossPercent ?? DEFAULT_PRODUCT_LOSS_CONFIG.retailLossPercent}
                    onChange={(e) => {
                      const val = parseFloat(e.target.value) || 0;
                      updateField('productLossConfig', {
                        ...DEFAULT_PRODUCT_LOSS_CONFIG,
                        ...formData.productLossConfig,
                        retailLossPercent: Math.min(50, Math.max(0, val)),
                      } as ProductLossConfig);
                    }}
                  />
                  <span className="text-xs text-muted-foreground">%</span>
                ...
                <p className="text-[10px] text-muted-foreground">
                  Expired, damaged, unsold stock
                  <span className="block mt-0.5 text-muted-foreground/70">
                    Source: Beverage industry average (WRAP, FAO)
                  </span>
                </p>
              </div>
            )}
            ...
            {boundaryNeedsEndOfLife(formData.systemBoundary) && (
              <div className="space-y-1.5">
                <Label htmlFor="consumerWaste" className="text-xs">
                  Consumer waste
                </Label>
                <div className="flex items-center gap-2">
                  <Input
                    ...
                    step={0.5}
                    className="w-20"
                    value={formData.productLossConfig?.consumerWastePercent ?? categoryDefault}
                    onChange={(e) => {
                      const val = parseFloat(e.target.value) || 0;
                      updateField('productLossConfig', {
                        ...DEFAULT_PRODUCT_LOSS_CONFIG,
                        ...formData.productLossConfig,
                        consumerWastePercent: Math.min(50, Math.max(0, val)),
                      } as ProductLossConfig);
                    }}
                  />
                  <span className="text-xs text-muted-foreground">%</span>
                ...
                <p className="text-[10px] text-muted-foreground">
                  Unconsumed product poured away
                  {wasteEntry && (
                    <span className="block mt-0.5 text-muted-foreground/70">
                      Source: {wasteEntry.source}
                    </span>
                  )}
                </p>
              </div>
          ...
          <button
            ...
            className="flex items-center gap-1.5 text-[11px] text-muted-foreground/70 hover:text-muted-foreground transition-colors mt-2"
          >
            <BookOpen className="h-3 w-3" />
            <span>Where do these defaults come from?</span>
            <ChevronDown className={cn('h-3 w-3 transition-transform', showSources && 'rotate-180')} />
          </button>
          {showSources && (
            <div className="rounded-md bg-muted/30 p-3 text-[11px] text-muted-foreground space-y-2 mt-1">
              <p>
                Default loss rates are drawn from published lifecycle assessment (LCA) data
                and industry research. Where direct studies exist, the confidence is high.
                Where data is estimated by analogy from related categories, the confidence is
                lower. You can override any value with your own primary data.
              </p>
              <div className="space-y-1">
                <p className="font-medium text-foreground/70">Key sources:</p>
                <ul className="list-inside list-disc space-y-0.5 text-muted-foreground/80">
                  <li>EU PEFCR Beer (2018), The Brewers of Europe</li>
                  <li>WRAP Household Food & Drink Waste in the UK (2012, 2022)</li>
                  <li>WRAP / Love Food Hate Waste "Down the Drain" campaign data</li>
                  <li>MDPI: Product Loss Rates in Wine Closure Systems (Sustainability, 2012, 4(10), 2673)</li>
                  <li>Oregon DEQ: Wine Product Environmental Footprint Literature Summary</li>
                  <li>BIER: Carbon Footprint of Spirits (Beverage Industry Environmental Roundtable)</li>
                  <li>EU PEFCR Packaged Water (2018), European Commission</li>
                </ul>
              </div>
              {wasteEntry && (
                <p className="pt-1 border-t border-muted-foreground/10">
                  <span className="font-medium text-foreground/70">{productCategory}:</span>{' '}
                  {wasteEntry.source}{' '}
                  <span className={cn(
                    'inline-flex items-center rounded px-1 py-0.5 text-[10px] font-medium',
                    wasteEntry.confidence === 'high' && 'bg-emerald-500/10 text-emerald-600 dark:text-emerald-400',
                    wasteEntry.confidence === 'medium' && 'bg-amber-500/10 text-amber-600 dark:text-amber-400',
                    wasteEntry.confidence === 'low' && 'bg-orange-500/10 text-orange-600 dark:text-orange-400',
                  )}>
                    {wasteEntry.confidence} confidence
                  </span>
                </p>
              )}
            </div>
