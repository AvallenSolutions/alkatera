...
interface ArableFieldCardProps {
  ...
  harvestCount?: number;
  onEdit: (field: ArableField) => void;
  onDelete: (id: string) => void;
  onEditProfile?: (field: ArableField) => void;
}
...
export function ArableFieldCard({ field, hasGrowingProfile, harvestCount, onEdit, onDelete, onEditProfile }: ArableFieldCardProps) {
  const cert = CERTIFICATION_LABELS[field.certification] || CERTIFICATION_LABELS.conventional;
  const typeLabel = CROP_TYPE_LABELS[field.crop_type] || field.crop_type;
    ...
    <Card className="bg-card border-border hover:border-[#ccff00]/30 transition-colors">
              ...
              <div>
                ...
                <div className="flex items-center gap-2 mt-1 text-sm text-muted-foreground">
                  ...
                  {sowingLabel && (
                    <>
                      <span>&middot;</span>
                      <span>{sowingLabel}</span>
                    </>
                  )}
                  {field.seed_rate_kg_per_ha != null && (
                    <>
                      <span>&middot;</span>
                      <span>{field.seed_rate_kg_per_ha} kg/ha seed</span>
                    </>
                  )}
                </div>
                <div className="flex items-center gap-2 mt-1 text-sm text-muted-foreground">
                  <span>
                    {field.annual_yield_tonnes
                      ? `${field.annual_yield_tonnes} t/year`
                      : 'Yield not set'}
                  </span>
                </div>
                {field.address_city && (
                  <div className="flex items-center gap-1 mt-1 text-xs text-muted-foreground">
                    ...
                    <span>
                      {[field.address_city, field.address_country].filter(Boolean).join(', ')}
                    </span>
                  </div>
                )}
                <div className="flex items-center gap-2 mt-2 flex-wrap">
                  ...
                  {field.crop_varieties && field.crop_varieties.length > 0 && (
                    <span className="text-xs text-muted-foreground">
                      {field.crop_varieties.slice(0, 3).join(', ')}
                      {field.crop_varieties.length > 3 && ` +${field.crop_varieties.length - 3}`}
                    </span>
                  )}
                </div>
                {(field as any).facilities && (
                  <div className="text-xs text-muted-foreground mt-1">
                    Linked to: {(field as any).facilities.name}
                  </div>
                )}
              </div>
            </div>
      ...
      <div className="px-4 pb-4 pt-0 flex items-center justify-between border-t border-border/50 mt-0 pt-3">
        <Button
          ...
          asChild
        >
          <Link href={`/arable-fields/${field.id}`}>
            View Dashboard
