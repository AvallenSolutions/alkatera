...
/**
 * Arable field trends, re-cut for the studio: cream hairline panels
 * with dim mono labels, studio inks for the series, a cream tooltip,
 * no icon titles and no gradients. The data series are unchanged.
 */
...
const COLOURS = {
  ...
  purple: STUDIO.plum,
  teal: STUDIO.dim,
  orange: STUDIO.ochreInk,
  cyan: STUDIO.forest,
} as const;

interface ArableTrendChartsProps {
...
export function ArableTrendCharts({ harvestImpacts }: ArableTrendChartsProps) {
  if (harvestImpacts.length < 2) {
    return (
      <div className="pt-2">
        <p className="text-sm text-muted-foreground">
          At least two harvests are needed to draw trends. Add another harvest to see the picture.
        </p>
      </div>
    );
  }
  ...
  const sourceData = harvestImpacts.map((v) => {
    ...
    return {
      ...
      'Grain Drying': imp.non_flag_emissions.grain_drying_co2e / area,
      Seed: imp.non_flag_emissions.seed_production_co2e / area,
  ...
  const n2oBreakdownData = harvestImpacts.map((v) => {
    ...
    return {
      ...
      'Crop residue (straw)': imp.flag_emissions.n2o_crop_residue_co2e / area,
      'Lime CO₂': imp.flag_emissions.lime_co2e / area,
    };
  });
  ...
  return (
    <div className="grid gap-4 md:grid-cols-2">
        ...
        <div className="mt-3">
          <ResponsiveContainer width="100%" height={250}>
            <AreaChart data={sourceData} margin={{ top: 5, right: 20, left: 10, bottom: 5 }}>
              ...
              <Area type="monotone" dataKey="Grain Drying" stackId="1" stroke={COLOURS.orange} fill={COLOURS.orange} fillOpacity={0.6} />
              <Area type="monotone" dataKey="Seed" stackId="1" stroke={COLOURS.cyan} fill={COLOURS.cyan} fillOpacity={0.6} />
      ...
      {/* 3. FLAG emissions breakdown (N2O + lime) */}
      <Panel>
        <Eyebrow tone="dim">FLAG EMISSIONS BREAKDOWN · PER HA</Eyebrow>
        <div className="mt-3">
          <ResponsiveContainer width="100%" height={250}>
            <BarChart data={n2oBreakdownData} margin={{ top: 5, right: 20, left: 10, bottom: 5 }}>
              ...
              <Bar dataKey="Crop residue (straw)" stackId="1" fill={COLOURS.teal} radius={[0, 0, 0, 0]} maxBarSize={40} />
              <Bar dataKey="Lime CO₂" stackId="1" fill={COLOURS.purple} radius={[2, 2, 0, 0]} maxBarSize={40} />
            </BarChart>
          </ResponsiveContainer>
        </div>
      </Panel>
        ...
        <div className="mt-3">
          <ResponsiveContainer width="100%" height={250}>
            <LineChart data={yieldData} margin={{ top: 5, right: 20, left: 10, bottom: 5 }}>
              ...
              <Line yAxisId="right" type="monotone" dataKey="emissions_per_tonne" name="kg CO₂e/t grain" stroke={COLOURS.red} strokeWidth={2} strokeDasharray="5 5" dot={{ r: 3, fill: COLOURS.red }} />
            </LineChart>
          </ResponsiveContainer>
        </div>
      </Panel>
