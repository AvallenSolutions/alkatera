...
// ─── Main ProductGuide Component ─────────────────────────────────────────────

interface ProductGuideProps {
  onAction?: (action: string) => void
}

export function ProductGuide({ onAction }: ProductGuideProps) {
  const { state, markProductGuideCompleted } = useOnboarding()
  ...
  const visibleSteps = useMemo(() => getVisibleProductSteps(isMobile), [isMobile])
  ...
  // Auto-start on first visit if guide not yet completed
  useEffect(() => {
    if (!state.productGuideCompleted) {
  ...
  const handleAction = useCallback(
    ...
    [markProductGuideCompleted, onAction]
  )
  ...
  // Public method to restart the tour via event
  ...
  useEffect(() => {
    ...
    window.addEventListener('product-guide:start', handler)
    return () => window.removeEventListener('product-guide:start', handler)
  }, [startTour])
  ...
  const getPopoverStyle = (): React.CSSProperties => {
    ...
    const cardWidth = 384
    const gap = 16
    const cardHeight = 300 // estimated max card height
    const viewportBottom = window.innerHeight

    switch (currentStep.popoverPosition) {
      case 'right': {
        const proposedTop = Math.max(20, targetRect.top - padding)
        const clampedTop = Math.min(proposedTop, viewportBottom - cardHeight - 20)
        return {
          position: 'fixed',
          top: Math.max(20, clampedTop),
      ...
      default: {
        ...
        const aboveTop = targetRect.top - padding - gap - cardHeight
        // Prefer below; flip to above if it would overflow; otherwise clamp
        let finalTop: number
        if (belowTop + cardHeight <= viewportBottom - 20) {
          finalTop = belowTop
        } else if (aboveTop >= 20) {
          finalTop = aboveTop
        } else {
          finalTop = Math.max(20, viewportBottom - cardHeight - 20)
        }
        return {
          position: 'fixed',
          top: finalTop,
...
export function ProductGuideTrigger() {
  return (
    <Button
      variant="outline"
      size="sm"
