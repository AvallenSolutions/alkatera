...
interface QuestionnaireProps {
  ...
  orchardCountryCode: string | null;
  orchardPreviousLandUse?: PreviousLandUseType | null;
  orchardLandConversionYear?: number | null;
  existingProfile?: OrchardGrowingProfile | null;
  harvestYear?: number;
  copyFromData?: Record<string, any>;
  onComplete: (profile: OrchardGrowingProfile) => void;
  onCancel: () => void;
}

const PREVIOUS_LAND_USE_OPTIONS: { value: PreviousLandUseType; label: string }[] = [
  { value: 'permanent_orchard', label: 'Already an orchard (no land use change)' },
...
const PESTICIDE_TYPES: { value: OrchardPesticideType; label: string }[] = [
  { value: 'generic', label: 'Generic / mixed products' },
  { value: 'sulfur', label: 'Sulfur-based' },
  { value: 'mancozeb', label: 'Mancozeb' },
  { value: 'synthetic_fungicide', label: 'Synthetic fungicide' },
  { value: 'insecticide_codling_moth', label: 'Codling moth insecticide' },
  { value: 'insecticide_aphid', label: 'Aphid insecticide' },
];

const HERBICIDE_TYPES: { value: OrchardPesticideType; label: string }[] = [
...
const STEPS = [
  ...
  { id: 'inputs', label: 'Inputs', icon: TreePine },
  { id: 'machinery', label: 'Machinery & Fuel', icon: Fuel },
  { id: 'irrigation', label: 'Irrigation & Transport', icon: Droplets },
] as const;
...
const SOIL_PRACTICES: { value: SoilManagement; label: string; description: string }[] = [
  ...
  { value: 'cover_cropping', label: 'Cover cropping (active)', description: 'Living ground cover between tree rows year-round' },
  { value: 'composting', label: 'Composting (active)', description: 'Regular compost application to orchard soils' },
...
export function OrchardGrowingQuestionnaire({
  orchardId,
  orchardName,
  orchardHectares,
  orchardType,
  ...
  orchardCertification,
  orchardCountryCode,
  orchardPreviousLandUse,
  orchardLandConversionYear,
  existingProfile,
  harvestYear,
  copyFromData,
  onComplete,
  ...
  const { awareFactor: orchardAwareFactor } = useAwareFactor(orchardCountryCode);
  ...
  const [form, setForm] = useState({
    ...
    pesticide_type: (initSource?.pesticide_type ?? 'generic') as OrchardPesticideType,
    ...
    herbicide_type: (initSource?.herbicide_type ?? 'generic') as OrchardPesticideType,
    ...
    // Land use change (FLAG-C3) - stored on orchard, not growing profile
    previous_land_use_type: (orchardPreviousLandUse ?? 'permanent_orchard') as PreviousLandUseType,
    land_conversion_year: orchardLandConversionYear ?? null as number | null,
  ...
  useEffect(() => {
    ...
    fetch(`/api/orchards/${orchardId}/growing-profile/evidence?profile_id=${existingProfile.id}`)
      .then((res) => res.json())
  ...
  async function handleSave(asDraft: boolean) {
    ...
    if (!asDraft) {
      if (form.fruit_yield_tonnes <= 0) {
        toast.error('Please enter the annual fruit yield (tonnes) to finalise');
        return;
      }
    ...
    setSaving(true);
    try {
      const url = `/api/orchards/${orchardId}/growing-profile`;
      ...
      const body = {
        ...
        fruit_yield_tonnes: form.fruit_yield_tonnes,
      ...
      // Save LUC fields to the orchard record (not the growing profile)
      const lucChanged =
        form.previous_land_use_type !== (orchardPreviousLandUse ?? 'permanent_orchard') ||
        form.land_conversion_year !== orchardLandConversionYear;
      if (lucChanged) {
        await fetch(`/api/orchards/${orchardId}`, {
          method: 'PATCH',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
            previous_land_use_type: form.previous_land_use_type === 'permanent_orchard'
              ? null
              : form.previous_land_use_type,
            land_conversion_year: form.previous_land_use_type === 'permanent_orchard'
              ? null
      ...
      if (evidenceFile && data?.id) {
        ...
        try {
          ...
          const evidenceRes = await fetch(
            `/api/orchards/${orchardId}/growing-profile/evidence`,
  ...
  // Preview calculation
  const previewResult = calculateOrchardImpacts({
    orchard_type: orchardType,
    climate_zone: orchardClimateZone,
    certification: orchardCertification,
    location_country_code: orchardCountryCode,
    ...
    <div className="space-y-4">
      ...
      <div className="space-y-2">
        <div className="flex items-center justify-between text-sm">
          <span className="text-muted-foreground">
            Fruit growing questionnaire for <span className="text-foreground font-medium">{orchardName}</span>
          </span>
          <span className="text-muted-foreground">
      ...
      <Card>
        <CardContent className="pt-6">
            ...
            <div className="space-y-4">
              <div>
                ...
                <p className="text-sm text-muted-foreground mt-1">
                  How you manage your orchard soils affects carbon sequestration and N2O emissions.
                </p>
              </div>
              ...
              <div className="grid gap-4">
                <div className="grid grid-cols-2 gap-4">
                  <div className="grid gap-2">
                    ...
                    <p className="text-xs text-muted-foreground">
                      Orchard total: {orchardHectares} ha. Enter the area used for this product.
                    </p>
                    {form.area_ha > orchardHectares && (
                      <p className="text-xs text-amber-600 dark:text-amber-400">
                        Area exceeds orchard total ({orchardHectares} ha)
                      </p>
                    )}
                  </div>
                  <div className="grid gap-2">
                    <Label htmlFor="yield">Annual fruit yield (tonnes) *</Label>
                    <Input
                      id="yield"
                      type="number"
                      step="0.1"
                      min="0.1"
                      value={form.fruit_yield_tonnes || ''}
                      onChange={(e) => updateForm({ fruit_yield_tonnes: parseFloat(e.target.value) || 0 })}
                    />
                    <p className="text-xs text-muted-foreground">
                      Total harvest from this area. UK apple average: 15-30 t/ha.
                    </p>
                    {form.fruit_yield_tonnes > 0 && form.area_ha > 0 && (
                      <>
                        <p className="text-xs text-muted-foreground">
                          Calculated yield: {(form.fruit_yield_tonnes / form.area_ha).toFixed(1)} t/ha
                        </p>
                        {form.fruit_yield_tonnes / form.area_ha > 60 && (
                          <p className="text-xs text-amber-600 dark:text-amber-400">
                            Yield above 60 t/ha is unusually high. Please check your figures.
                          </p>
                        )}
                        {form.fruit_yield_tonnes / form.area_ha < 1 && (
                          <p className="text-xs text-amber-600 dark:text-amber-400">
                            Yield below 1 t/ha is very low. Is this a newly planted orchard?
                          </p>
                        )}
                      </>
                    )}
                ...
                <div className="grid gap-2">
                  ...
                  <p className="text-xs text-muted-foreground">
                    How tree prunings are managed after pruning. This affects the N2O calculation.
                  </p>
                  <Select
                ...
                <div className="space-y-3">
                  ...
                  <div className="grid gap-3">
                    <div className="grid gap-1.5">
                      ...
                      <p className="text-xs text-muted-foreground">
                        Select the dominant ecosystem type at your orchard's location. This is used for TNFD nature impact disclosure.
                      </p>
                      <Select
                      ...
                      <>
                        <div className="grid gap-1.5">
                          ...
                          <Input
                            ...
                            placeholder="e.g. Kent Downs AONB"
                            className="h-9"
                          />
                        </div>
                ...
                <div className="grid gap-4">
                  <div>
                    ...
                    <p className="text-xs text-muted-foreground">
                      What was this land used for before it became an orchard?
                  ...
                  {form.previous_land_use_type !== 'permanent_orchard' && (
                    <div className="grid gap-2 max-w-[200px]">
                      <Label htmlFor="conversion-year">Year of conversion to orchard</Label>
                      <Input
                        id="conversion-year"
                        type="number"
                        min="1900"
                              ...
                              <button
                                ...
                                onClick={async () => {
                                  const res = await fetch(
                                    `/api/orchards/${orchardId}/growing-profile/evidence?evidence_id=${doc.id}`,
                                    { method: 'DELETE' }
                                  );
                                  if (res.ok) {
                    ...
                    <div className="grid gap-2">
                      ...
                      {form.fertiliser_quantity_kg > 0 && form.area_ha > 0 && (
                        <p className="text-xs text-muted-foreground">
                          ...
                          {form.fertiliser_type === 'synthetic_n' && form.fertiliser_quantity_kg / form.area_ha > 250 && (
                            <span className="text-amber-600 dark:text-amber-400 ml-1">
                              High for orchards (typical: 50-150 kg N/ha)
                            </span>
                          )}
                        </p>
                      )}
                ...
                <div className="space-y-3">
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <p className="text-xs text-muted-foreground">
                        Includes sulfur, mancozeb, and insecticide treatments
                      </p>
                    </div>
                    <Switch
                    ...
                    <div className="grid gap-3 pl-4">
                      ...
                      <div className="grid gap-2">
                        ...
                        <Select
                          ...
                          onValueChange={(v) => updateForm({ pesticide_type: v as OrchardPesticideType })}
                        >
                    ...
                    <div className="grid gap-3 pl-4">
                      ...
                      <div className="grid gap-2">
                        ...
                        <Select
                          ...
                          onValueChange={(v) => updateForm({ herbicide_type: v as OrchardPesticideType })}
                        >
                          <SelectTrigger className="w-full">
                            <SelectValue />
                          </SelectTrigger>
          ...
          {currentStep === 2 && (
            <div className="space-y-4">
              <div>
                ...
                <p className="text-sm text-muted-foreground mt-1">
                  Diesel consumption for tractors, sprayers, mowers, and harvesting equipment.
                </p>
              </div>
              ...
              <div className="grid gap-4">
                <div className="grid grid-cols-2 gap-4">
                  <div className="grid gap-2">
                    ...
                    />
                    <p className="text-xs text-muted-foreground">
                      Total diesel for all orchard operations. Sector average: 100-200 L/ha/yr.
                    </p>
                    {form.diesel_litres_per_year > 0 && form.area_ha > 0 && (
                      <p className="text-xs text-muted-foreground">
                        {(form.diesel_litres_per_year / form.area_ha).toFixed(0)} L/ha
                        {form.diesel_litres_per_year / form.area_ha > 400 && (
                          <span className="text-amber-600 dark:text-amber-400 ml-1">
                            Above 400 L/ha is unusually high
                          </span>
                        )}
                      </p>
                    )}
          ...
          {/* Step 4: Irrigation & Transport */}
          {currentStep === 3 && (
            <div className="space-y-4">
              <div>
                <h3 className="font-semibold text-lg">Irrigation & Transport</h3>
                <p className="text-sm text-muted-foreground mt-1">
                  Water use, pumping energy, and transport from orchard to processing facility.
                </p>
              </div>
              ...
              <div className="grid gap-4">
                <div className="flex items-center justify-between">
                  <div>
                    <Label>Orchard is irrigated</Label>
                    <p className="text-xs text-muted-foreground">
                      Toggle off if your orchard is entirely rainfed
                    </p>
                  </div>
                  <Switch
                ...
                <div className="grid gap-4">
                  <div>
                    ...
                    <p className="text-xs text-muted-foreground mt-1">
                      Distance from orchard to the facility where the fruit is processed (e.g. cider mill, distillery).
                    </p>
                  </div>
                  <div className="grid grid-cols-2 gap-4">
                    <div className="grid gap-2">
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div className="rounded-lg border border-border p-3">
                    ...
                    <div className="text-xs text-muted-foreground mt-1">
                      {previewResult.total_emissions_per_kg.toFixed(3)} kg CO2e per kg fruit
                    </div>
                  </div>
