 ...
 *
 * A room with no entry (today) gets no panel at all: its band coverage is
 * already handled by the desk priorities. The library has an entry with
 * an empty band list — intro only, no checklist, because its "room" has
 * no growth-score signals of its own.
 *
