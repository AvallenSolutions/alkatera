...
export interface ArableCalculatorInput {
   ...
   * soil_carbon_samples and cached on the growing profile. Already net of the
   * conservative confidence discount and never negative. Takes priority over the
   * manual override and practice-based defaults when present.
   */
  soil_carbon_annual_change_kg_co2e_per_ha?: number | null;
  /** Methodology that produced the stock-change flux (for provenance) */
  soil_carbon_change_methodology?: string | null;
  /** Confidence grade of the measured stock-change */
  soil_carbon_change_confidence?: 'HIGH' | 'MEDIUM' | 'LOW' | null;
