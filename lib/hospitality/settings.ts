...
export type HospitalitySection = 'dashboard' | 'venues' | 'meals' | 'drinks' | 'menus' | 'rooms' | 'sales' | 'waste' | 'events' | 'marketplace' | 'integrations'
...
export function hospitalitySectionFromHref(href: string): HospitalitySection | null {
  ...
  const seg = m[1]
  if (['venues', 'meals', 'drinks', 'menus', 'rooms', 'sales', 'waste', 'events', 'marketplace', 'integrations'].includes(seg)) return seg as HospitalitySection
  return null
}
