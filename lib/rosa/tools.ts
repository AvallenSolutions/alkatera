...
export const ROSA_TOOLS: ToolDefinition[] = [
  ...
  },
  {
    name: 'get_next_asks',
    description:
      "Returns the organisation's top Ask Queue items (tasks/data-revolution-plan.md Pillar 3) — small, plain-language questions the platform generated from real data gaps (an estimated ingredient factor, an unconfirmed recipe, an estimated utility bill, an implausible-looking figure, an undone setup step), ordered by impact on footprint accuracy where computable. Each item has an id, a question, an answer_shape ('number' | 'confirm_value' | 'yes_no' | 'choice' | 'link'), options when relevant, and impact_share (0-1, or null when not computable). Use when the user asks 'what should I answer next?', 'any quick questions for me?', or 'walk me through the queue'. Answer one with propose_answer_ask (except answer_shape 'link' items, which the user must open via their href — point them at the queue instead).",
    input_schema: {
      type: 'object',
      properties: {
        limit: { type: 'number', description: 'Max items to return (default 5, hard cap 20).' },
      },
      required: [],
    },
  },
  ...
  {
    name: 'propose_answer_ask',
    description:
      "Propose an answer to one item from get_next_asks. Stages a pending action; the user clicks Confirm to write it. The `answer` shape must match the item's answer_shape: a number for 'number', true for 'confirm_value' (there is no 'no' — if the user thinks it's wrong, point them at the item's href instead and don't call this), true/false for 'yes_no', or the exact option value for 'choice'. Never call this for an answer_shape 'link' item — those have no field to write; tell the user to follow the href.",
    input_schema: {
      type: 'object',
      properties: {
        exception_id: { type: 'string', description: 'The agent_exceptions.id of the ask, from get_next_asks.' },
        answer: {
          type: 'string',
          description:
            "The answer as a string, shaped per the ask's answer_shape: a number written as digits (e.g. '12.5') for 'number', 'true' for 'confirm_value', 'true' or 'false' for 'yes_no', or the exact choice option value for 'choice'.",
        },
      },
      required: ['exception_id', 'answer'],
    },
  },
  {
...
export async function executeTool(
  ...
  try {
    switch (name) {
      ...
      case 'get_next_asks':
        return await toolGetNextAsks(ctx, input as { limit?: number });
...
/**
 * The Ask Queue (Pillar 3): top open asks ordered by payload.priority_score
 * (lib/asks/impact.ts), same ordering the desk's one-tap ask card and the
 * full queue view use. 'ask' rows are generated by lib/asks/generate.ts's
 * sweepAsks — a fresh org may have none yet, which is a normal, quiet
 * result, not an error.
 */
async function toolGetNextAsks(ctx: ToolContext, input: { limit?: number }): Promise<ToolResult> {
  ...
  const { data, error } = await ctx.supabase
    ...
    .limit(100);
  if (error) {
    return { is_error: true, content: error.message, audit: { tool: 'get_next_asks', error: error.message } };
  }
  ...
  const rows = (data ?? [])
    .map((r: any) => ({
      ...
      answer_shape: r.payload?.answer_shape,
      options: r.payload?.options,
      current_value: r.payload?.current_value,
      unit: r.payload?.unit,
      impact_share: r.payload?.impact_share,
      href: r.payload?.href,
      priority_score: r.payload?.priority_score ?? -1,
    }))
    .sort((a: any, b: any) => b.priority_score - a.priority_score)
    .slice(0, limit)
    .map(({ priority_score, ...rest }: any) => rest);

  return {
    is_error: false,
    content: JSON.stringify({ asks: rows, count: rows.length }),
    audit: { tool: 'get_next_asks', count: rows.length },
  };
}
...
function buildActionPreview(toolName: ActionToolName, p: Record<string, unknown>): string {
  switch (toolName) {
    ...
    case 'propose_answer_ask': {
      return `Answer "${String(p.answer)}" to queue item ${String(p.exception_id).slice(0, 8)}.`;
    }
    case 'propose_match_emission_factor': {
...
function validateActionInput(toolName: ActionToolName, input: Record<string, unknown>): string[] {
  const required: Record<ActionToolName, string[]> = {
    ...
    propose_answer_ask: ['exception_id', 'answer'],
  };
