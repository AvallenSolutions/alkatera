...
export function validateImportRow(row: Record<string, string>, rowIndex: number): { valid: boolean; errors: string[] } {
  const errors: string[] = [];
  if (!row.name || row.name.trim() === '') errors.push(`Row ${rowIndex}: Name is required`);
  if (row.quantity && isNaN(Number(row.quantity))) errors.push(`Row ${rowIndex}: Quantity must be a number`);
