/**
 * Fruit Orchard Impact Calculator
 *
 * Calculates the environmental impact of orchard operations (fruit growing)
 * for producers who grow their own agricultural inputs (apples, pears, etc.).
 *
 ...
 *   6. Transport from orchard to processing facility (DEFRA tonne-km)
 *
...
function calculateLUC(
  ...
  if (!previousLandUse || previousLandUse === 'permanent_orchard') return 0;
  ...
  const currentStock = ORCHARD_CARBON_STOCK[orchardType]?.[climateZone];
...
/**
 * Calculate orchard impacts for a growing profile.
 *
...
export function calculateOrchardImpacts(
  ...
  // ========================================================================
  // 1b. CROP RESIDUE N2O (FLAG: tree prunings returned to soil)
  // ========================================================================
  ...
  if (input.uses_pesticides && input.pesticide_applications_per_year > 0) {
    ...
    const pesticideProfile = ORCHARD_PESTICIDE_ECOTOX_PROFILES[input.pesticide_type || 'generic'];
  ...
  if (input.uses_herbicides && input.herbicide_applications_per_year > 0) {
    ...
    const herbicideProfile = ORCHARD_PESTICIDE_ECOTOX_PROFILES[input.herbicide_type || 'generic'];
  ...
  if (input.transport_distance_km && input.transport_distance_km > 0 && input.fruit_yield_tonnes > 0) {
    const mode = input.transport_mode || 'road';
    const ef = ORCHARD_TRANSPORT_EF[mode] ?? ORCHARD_TRANSPORT_EF.road;
    // kg CO2e = tonnes * km * factor (kg CO2e per tonne-km)
    transportCo2e = input.fruit_yield_tonnes * input.transport_distance_km * ef;
  }
