...
/**
 * Convert a workbook to a concise text representation for the model.
...
export async function parseOrchardSprayDiary(fileBuffer: Buffer): Promise<OrchardSprayChemicalDraft[]> {
  ...
  const raw = (await runTextPrompt({
    apiKey,
    maxTokens: 4096,
    op: 'orchard_spray_diary',
    prompt: `You are analysing a fruit orchard spray diary spreadsheet (apples, pears, cherries, stone fruit, citrus). Extract all chemical/product applications and return structured JSON.
