...
const COBALT = '#2B46C0';
const OCHRE = '#DFA32B';
const OCHRE_INK = '#A97C14'; // ochre's accent form on paper (contrast)
const BRICK = '#BF4B2A';
const GOOD = '#047857';
const ATTN = '#B45309';
const STALE = '#BE123C';
...
/** Text colours on a saturated band: cream everywhere, ink on ochre (always). */
function onBand(bg: string): { fg: string; meta: string; hairline: string } {
  if (bg === OCHRE) return { fg: INK, meta: 'rgba(26,27,29,.7)', hairline: 'rgba(26,27,29,.2)' };
...
function wordmark(size: number, color: string): string {
...
function leafMark(stroke: string, style: string): string {
...
function mono(text: string, opts: { size?: number; ls?: number; color?: string; weight?: number; upper?: boolean } = {}): string {
...
function band(number: string, title: string, meta: string, bg: string): string {
  const on = onBand(bg);
...
function heroBand(number: string, title: string, meta: string, bg: string, inner: string): string {
  const on = onBand(bg);
...
function statCard(label: string, value: string, unit?: string, opts: { valueColor?: string; size?: number; pad?: number } = {}): string {
...
function cardTitle(text: string, metaSuffix?: string): string {
...
function bodyP(text: string, opts: { color?: string; size?: number; mt?: number; maxWidth?: number } = {}): string {
...
function defBlock(title: string, body: string, metaSuffix?: string): string {
  return `<div>${cardTitle(title, metaSuffix)}${bodyP(body)}</div>`;
}

/** Square bullet row: filled (included) or outlined (excluded/limitation). */
function bullet(text: string, opts: { color?: string; outline?: boolean; textColor?: string } = {}): string {
...
function th(text: string, opts: { align?: 'left' | 'right'; first?: boolean; last?: boolean; width?: number; size?: number } = {}): string {
...
function td(content: string, opts: { align?: 'left' | 'right'; first?: boolean; last?: boolean; dim?: boolean; bold?: boolean; size?: number; mono?: boolean; color?: string; top?: boolean } = {}): string {
...
function toneChip(label: string, tone: 'good' | 'attention' | 'stale' | 'quiet', size = 9.5): string {
...
function gradeTone(grade: string): 'good' | 'attention' | 'stale' {
...
function meter(pct: number, fill: string, opts: { height?: number; track?: string; mt?: number } = {}): string {
...
/**
 * One fixed A4 page (794×1123 @ 96dpi). Paper ground, the design's footer
 * (GENERATED BY wordmark · ISO badge + date) pinned to the bottom hairline.
 */
function page(content: string, opts: { footer?: boolean } = {}): string {
  ...
  const footerHtml = footer
    ? `<div style="position:absolute;left:44px;right:44px;bottom:26px;z-index:10;background:${PAPER};display:flex;justify-content:space-between;align-items:baseline;border-top:1px solid ${HAIR};padding-top:8px">
        ...
        <div style="font-family:${MONO};font-size:9px;font-weight:500;letter-spacing:.18em;color:${DIM}">ISO 14040/44 &amp; 14067 · ${dateStr}</div>
      </div>`
    : '';
